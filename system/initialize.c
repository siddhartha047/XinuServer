...
//sid: defined from paging.h 
frame_t frame_tab[NFRAMES];
frame_t *frame_head;
...
static void initialize_paging()
{
	...
	XDEBUG_KPRINTF("Setting up Page fault handler\n");
	set_evec(14,(uint32)pagefault);
	
	XDEBUG_KPRINTF("Setting up page directory of NULL process\n");	
	set_page_directory(address_to_vpn((&proctab[NULLPROC])->prpd));
